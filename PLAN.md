...
## 3. 下一步建议顺序
1. **VFX / shader**：对象与全屏 shader 效果（震屏、模糊、滤镜）。
2. **自动/快进模式**：auto-play 与 skip，配合已有打字机与 backlog。
3. **ViewManager**：统一标题/游戏/存档/设置等视图切换。
4. **设置界面 + 快捷键**：文字速度、音量、跳过已读等。
5. **脚本热加载**：编辑剧本后免重启刷新流程图。
