...
UI / 系统（演出做完后）：
- [ ] 存档系统（建议按评论：全局存档拆成多文件，去掉 CheckpointBlock 链表）
- [ ] 存档界面
- [ ] 文本回顾界面（BacklogViewController）
- [ ] ViewManager
- [ ] 预加载系统 / 随意缩放窗口 / 对话框完整功能 / 鼠标菜单 / 警告框 / 通知框
- [ ] 设置界面 / 快捷键 / 图片鉴赏 / 音乐鉴赏 / 手柄支持 / 立绘裁剪工具

## 3. 下一步建议顺序
1. **立绘合成系统**：多图层（基础+表情+口型）合成到一个可动画对象。
2. **对话框文字动画**：逐字显示 + 跳过 + 结束图标。
3. **变量系统**：脚本可读写的持久状态，为存档铺路。
4. **存档系统**：基于可重放的 GameState 做 model 序列化（按评论拆多文件）。
5. **文本回顾 + ViewManager**：调试演出更方便。

## 4. 测试剧本
- `resources/scenarios/test_runtime.txt`：自检剧本，覆盖 show/cam/trans/branch/jump。
